...
// Subtract 2 weeks from the current date and time
$delayedDate = date('Y-m-d H:i:s', strtotime('-2 weeks', strtotime($currentDate))); // Subtract 2 weeks

// Get the month and year of the delayed date
$delayedMonth = date('m', strtotime($delayedDate)); // Month of the delayed date
$delayedYear = date('Y', strtotime($delayedDate));  // Year of the delayed date

// Check if the current evaluator has already evaluated this employee in the delayed month
$checkSql = "SELECT * FROM ptp_evaluations 
             WHERE evaluator_id = ? 
             AND employee_id = ? 
             AND MONTH(evaluated_at) = ? 
             AND YEAR(evaluated_at) = ?";
$checkStmt = $conn->prepare($checkSql);
$checkStmt->bind_param('ssii', $evaluatorId, $employeeId, $delayedMonth, $delayedYear);
$checkStmt->execute();
$checkStmt->store_result();

if ($checkStmt->num_rows > 0) {
    echo 'You have already evaluated this employee for the delayed month.';
} else {
    ...
    $stmt->bind_param(
        ...
        $delayedDate // Use the delayed date and time for the evaluation
    );

    // Execute the statement and check if successful
    if ($stmt->execute()) {
        echo 'Evaluation saved successfully for the delayed month.';
    } else {
